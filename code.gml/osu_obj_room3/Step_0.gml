...
if (osu_active == true) {
    ...
    if (keyboard_check_pressed(vk_space) || mouse_check_button_pressed(mb_left)) {
        ...
        // CHECK ACCACY: If the ring scale is resting cse to the ore ring size (1.0)
        if (osu_ring_scale >= 1.0 - osu_success_margin && osu_ring_scale <= 1.0 + osu_success_margin) {
            
            // PERFECT HI Shut off minigame loop and advance room
            osu_active = false;
			time_add(20);
            room_goto_next(); 
            ...
            osu_ring_scale = 3.0; 
            time_subtract(10); // Keep your cus time penalty hook here
        }
    }
}
